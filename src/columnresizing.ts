    ...
    props: {
      ...
      handleDOMEvents: {
        ...
        mousedown: (view, event) => {
          handleMouseDown(view, event, cellMinWidth, defaultCellMinWidth, overflow);
        },
      },
...
function handleMouseDown(
  ...
  const tableDom = view.nodeDOM(pluginState.activeHandle)?.parentElement?.parentElement as HTMLElement

  const $cell = view.state.doc.resolve(pluginState.activeHandle);
  const table = $cell.node(-1)
  const map = TableMap.get(table)
  const start = $cell.start(-1);
  const leftColCount = map.colCount($cell.pos - start) + $cell.nodeAfter!.attrs.colspan - 1;

  const colgroup = tableDom.parentElement?.firstChild as HTMLElement

  const editorDom = tableDom.parentElement?.parentElement?.parentElement as HTMLElement
  const editorCSS = window.getComputedStyle(editorDom)

  const eidtorWidth = editorDom.offsetWidth - transforPXtoNumber(editorCSS.paddingRight) - transforPXtoNumber(editorCSS.paddingLeft)
  ...
  function finish(event: MouseEvent) {
    ...
    if (pluginState?.dragging) {
      updateColumnWidth(
        ...
        draggedWidth(pluginState.dragging, event, cellMinWidth, tableDom, map.width - leftColCount - 1, colgroup.children[leftColCount] as HTMLElement, eidtorWidth, overflow),
      );
      view.dispatch(
  ...
  function move(event: MouseEvent): void {
    ...
    if (!pluginState) return;
    if (pluginState.dragging) {
      const dragged = draggedWidth(pluginState.dragging, event, cellMinWidth, tableDom, map.width - leftColCount - 1, colgroup.children[leftColCount] as HTMLElement, eidtorWidth, overflow);
      displayColumnWidth(
        view,
        pluginState.activeHandle,
        dragged,
...
function draggedWidth(
  ...
  if (offset <= 0 || overflow) {
    ...
    if (rightColCount === 0) {
      const maxOffset = eidtorWidth - cellDom.getBoundingClientRect().left + tableDom.getBoundingClientRect().left - 1
      return Math.min(maxOffset, Math.max(resizeMinWidth, dragging.startWidth + offset));
    }
    else if (cellDom.offsetWidth + cellDom.getBoundingClientRect().left - tableDom.getBoundingClientRect().left + rightColCount * resizeMinWidth === tableDom.offsetWidth) {
      return dragging.startWidth
    }

    const maxOffset =
      tableDom.offsetWidth - rightColCount * resizeMinWidth
...
function transforPXtoNumber(px: string) {
